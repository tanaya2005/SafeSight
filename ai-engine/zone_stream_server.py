...
VOICE_CONFIG_PATH = os.path.join(os.path.dirname(__file__), "voice_alerts_config.json")
try:
    with open(VOICE_CONFIG_PATH, 'r', encoding='utf-8') as f:
        VOICE_CONFIG = json.load(f)
except:
    VOICE_CONFIG = None
    print("⚠️  Voice alerts config not found")
...
VOICE_ALERT_LANGUAGE = os.getenv('VOICE_ALERT_LANG', 'en')  # Default to English
...
_voice_alert_cooldown = {}  # Track voice alert cooldowns
...
def trigger_voice_alert(zone_name, language=None):
    """
    Trigger voice alert for zone violation with cooldown
    
    Args:
        zone_name: Name of the zone
        language: Language code (en, hi, mr) - uses env default if None
    """
    if language is None:
        language = VOICE_ALERT_LANGUAGE
    
    # Check cooldown
    alert_key = f"{zone_name}_{language}"
    now = time.time()
    
    if alert_key in _voice_alert_cooldown:
        if now - _voice_alert_cooldown[alert_key] < VIOLATION_COOLDOWN:
            return  # Skip - too soon
    
    _voice_alert_cooldown[alert_key] = now
    
    # Get alert text from config
    if VOICE_CONFIG:
        try:
            text = VOICE_CONFIG["zone_alerts"][zone_name][language]
        except KeyError:
            # Fallback to English
            try:
                text = VOICE_CONFIG["zone_alerts"][zone_name]["en"]
            except KeyError:
                text = f"Warning! Worker entered {zone_name}"
    else:
        text = f"Warning! Worker entered {zone_name}"
    
    # Play voice alert in background thread
    def _play_alert():
        try:
            ...
            # Save to temp file and play (you can use pygame or playsound)
            temp_file = f"temp_alert_{int(time.time())}.mp3"
...
def send_zone_violation(cam_id, bbox, frame, worker_id, zone_name, severity):
    """Send zone intrusion alert to backend and trigger voice alert."""
...
@app.route("/api/audio")
def generate_audio():
    """Generate voice alerts in multiple languages for zone violations"""
